-- Recetario de Javi · cuentas, perfiles y recetas familiares
-- Ejecutar después de 001_recetas.sql en el SQL Editor de Supabase.
...
alter table public.recipes add column if not exists family_id uuid references public.recipe_families(id) on delete set null;
...
begin
  ...
  values (
    new.id,
    coalesce(nullif(trim(new.raw_user_meta_data ->> 'display_name'), ''), split_part(new.email, '@', 1), 'Familiar')
  )
...
insert into public.profiles (id, display_name)
select id, coalesce(nullif(trim(raw_user_meta_data ->> 'display_name'), ''), split_part(email, '@', 1), 'Familiar')
from auth.users
on conflict (id) do nothing;
...
drop policy if exists "Authenticated users read recipe profiles" on public.profiles;
create policy "Authenticated users read recipe profiles"
on public.profiles for select to authenticated
using (true);

drop policy if exists "Users update their recipe profile" on public.profiles;
create policy "Users update their recipe profile"
on public.profiles for update to authenticated
using (id = auth.uid())
with check (id = auth.uid());

drop policy if exists "Users create their recipe profile" on public.profiles;
...
using (exists (select 1 from public.recipes recipe where recipe.id = recipe_id and recipe.owner_id = auth.uid()))
with check (exists (select 1 from public.recipes recipe where recipe.id = recipe_id and recipe.owner_id = auth.uid()));
