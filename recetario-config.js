...
(() => {
  ...
  addStyle('layout-fixes.css?v=0.10.3', 'layout');
  addStyle('community.css?v=0.10.3', 'community');
  addStyle('confirmation-ui.css?v=0.10.3', 'confirmation-ui');
  addStyle('notifications.css?v=0.10.3', 'notifications');
  addStyle('family-ui-settings.css?v=0.10.3', 'family-ui-settings');
  addStyle('ai-recipe.css?v=0.10.3', 'ai-recipe');

  window.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.version-block strong').forEach(node => { node.textContent = config.version; });
    addScript('confirmation-ui.js?v=0.10.3', 'confirmation-ui');
  });

  window.addEventListener('load', () => {
    const path = window.location.pathname;
    if (/\/cuenta\.html$/i.test(path)) {
      addScript('registration-patch.js?v=0.10.3', 'registration-patch');
      addScript('account-header.js?v=0.10.3', 'account-header');
      addScript('family-actions.js?v=0.10.3', 'family-actions');
      addScript('family-ui-settings.js?v=0.10.3', 'family-ui-settings');
    } else if (/\/admin\.html$/i.test(path)) {
      addScript('admin-enhancements.js?v=0.10.3', 'admin-enhancements');
      addScript('admin-capacity.js?v=0.10.3', 'admin-capacity');
      addScript('family-ui-settings.js?v=0.10.3', 'family-ui-settings');
      addScript('ai-recipe.js?v=0.10.3', 'ai-recipe');
    } else {
      addScript('recipe-community.js?v=0.10.3', 'recipe-community');
      addScript('recipe-metadata.js?v=0.10.3', 'recipe-metadata');
      addScript('recipe-defaults.js?v=0.10.3', 'recipe-defaults');
      addScript('ai-recipe.js?v=0.10.3', 'ai-recipe');
    }
    addScript('notifications.js?v=0.10.3', 'notifications');
  });
